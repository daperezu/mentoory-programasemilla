...
-- Insert core dashboard widgets
IF NOT EXISTS (SELECT 1 FROM [core].[DashboardWidgets] WHERE [Name] = 'progress_overview')
BEGIN
    INSERT INTO [core].[DashboardWidgets] 
    ([Name], [DisplayName], [Description], [Type], [Component], [IconClass], [DefaultConfig], [Roles], [MinSize], [MaxSize])
    VALUES
...
-- Insert role dashboard templates
IF NOT EXISTS (SELECT 1 FROM [core].[RoleDashboardTemplates] WHERE [IsActive] = 1)
BEGIN
    INSERT INTO [core].[RoleDashboardTemplates] 
    ([Role], [RoleName], [DefaultLayout], [DefaultTheme], [DefaultLanguage], [DefaultRefreshInterval], [WidgetCodes], [IsActive], [CreatedDate])
    SELECT 
        r.Name,
        r.Name,
        CASE r.Name
        ...
        'light', -- DefaultTheme
        'es', -- DefaultLanguage
        300, -- DefaultRefreshInterval (seconds)
        CASE r.Name
...
IF NOT EXISTS (SELECT 1 FROM [core].[UserNotifications])
BEGIN
    -- Sample notifications will be created by the application
    PRINT 'Notifications will be created by application events';
END;
...
PRINT 'Dashboard seed data completed successfully';