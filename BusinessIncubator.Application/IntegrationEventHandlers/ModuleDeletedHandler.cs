...
/// <summary>
/// Handles the ModuleDeleted integration event to clear source references in project modules.
/// </summary>
public sealed class ModuleDeletedHandler(
    IBusinessIncubatorRepository repository,
    ILogger<ModuleDeletedHandler> logger) : INotificationHandler<ModuleDeleted>
{
    public async Task Handle(ModuleDeleted notification, CancellationToken cancellationToken)
    {
        logger.LogInformation(
            "Processing module deletion sync for ModuleId: {ModuleId}",
        ...
        try
        {
            ...
            foreach (var reference in projectModuleReferences)
            {
                ...
                module.ClearSource();
                logger.LogInformation(
                    "Cleared source reference for module {ModuleId} in project {ProjectId}",
                    reference.ModuleId,
                    reference.ProjectId);
            ...
            logger.LogInformation(
                "Successfully cleared source references for {Count} project modules for deleted ModuleId: {ModuleId}",
        ...
        catch (Exception ex)
        {
            logger.LogError(
                ex,
                "Error clearing source references for deleted ModuleId: {ModuleId}",
                notification.ModuleId);
            throw;
        }
    }
