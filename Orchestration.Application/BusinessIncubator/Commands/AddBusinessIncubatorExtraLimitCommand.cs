...
public record AddBusinessIncubatorExtraLimitCommand(Guid ExternalId, int Type, int Quantity) : IBaseRequest;

public class AddBusinessIncubatorExtraLimitsCommandValidator : AbstractValidator<AddBusinessIncubatorExtraLimitCommand>
{
    public AddBusinessIncubatorExtraLimitsCommandValidator()
    {
...
public class AddBusinessIncubatorExtraLimitCommandHandler(IMediator mediator) : BaseCommandHandler<AddBusinessIncubatorExtraLimitCommand>
{
    public override async Task<Result> Handle(AddBusinessIncubatorExtraLimitCommand request, CancellationToken cancellationToken)
    {
        ...
        var clearResult = await mediator.Send(new AddExtraLimitCommand(incubatorId, request.Type, request.Quantity), cancellationToken);
